...
I want to be really clear about what we mean when we talk about proving the security of a blockchain mechanism. The proofs from the Ouroboros paper are probably all correct, I will not be attempting to disprove any of them in this paper.
...
In this paper, I will be trying to explain what security model Ouroboros is based upon, and I will try to show that their security model is not good enough to actually secure a cryptocurrency.
...
So it is necessary to program in some fork choice rule so that we can deal with orphans.
...
So this shows that if a majority of block producers want to change the consensus rules, they can do a soft fork to make this change, and everyone else needs to follow their change.
