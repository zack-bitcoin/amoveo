...
handle_cast({get, Id, Msg}, X) ->
    {ok, InboxPerPeer} = application:get_env(ae_core, inbox_per_peer),
    {ok, MaxMessageSize} = application:get_env(ae_core, max_message_size),
    ...
    E = dict:erase(N - InboxPerPeer, D),
    S = size(Msg),
    true = S < MaxMessageSize,
