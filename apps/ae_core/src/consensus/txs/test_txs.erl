...
test(1) ->
    ...
    Stx3 = testnet_sign:sign_tx(Ctx3, NewPub, NewPriv, Accounts3),
    absorb(Stx3),
    ...
    Block = block:make(PH, Txs, constants:master_pub()),%1 is the master pub
...
test(3) ->
    ...
    Stx2 = keys:sign(Ctx2, Accounts2),
    SStx2 = testnet_sign:sign_tx(Stx2, NewPub, NewPriv, Accounts2), 
    absorb(SStx2),
    {Trees3, _, _} = tx_pool:data(),
    Accounts3 = trees:accounts(Trees3),
    ...
    SStx3 = testnet_sign:sign_tx(Stx3, NewPub, NewPriv, Accounts3),
    absorb(SStx3),
    {Trees4, _, _} = tx_pool:data(),
    Accounts4 = trees:accounts(Trees4),
...
test(5) -> 
    ...
    ScriptPubKey = keys:sign(spk:new(constants:master_pub(), NewPub, CID, [Bet], 10000, 10000, ChannelNonce+1, Delay, Entropy), Accounts3),
    SignedScriptPubKey = testnet_sign:sign_tx(ScriptPubKey, NewPub, NewPriv, Accounts3), 
...
test(6) -> 
    ...
    Stx4 = testnet_sign:sign_tx(Ctx4, NewPub, NewPriv, Accounts4),
    %Stx4 = keys:sign(Ctx4, Accounts4),
    absorb(Stx4),
...
test(10) ->
   %spend shares with channel, with solo_close
    ...
    ChannelNonce = 0,
    Bet = spk:new_bet(Code, Code, 50, []),
    ScriptPubKey = keys:sign(spk:new(NewPub, constants:master_pub(), CID, [Bet], 10000, 10000, ChannelNonce+1, Delay, Entropy), Accounts4),
    SignedScriptPubKey = testnet_sign:sign_tx(ScriptPubKey, NewPub, NewPriv, Accounts4), 
    ScriptSig = compiler_chalang:doit(<<" int 0 int 1 ">>),
...
test(12) ->
    ...
    ScriptPubKey = keys:sign(spk:new(constants:master_pub(), NewPub, CID, [Bet, Bet2], 10000, 10000, ChannelNonce+1, Delay, Entropy), Accounts3),
    SignedScriptPubKey = testnet_sign:sign_tx(ScriptPubKey, NewPub, NewPriv, Accounts3), 
    ScriptSig = compiler_chalang:doit(<<" int 0 int 1 ">>),
    ScriptSig2 = compiler_chalang:doit(<<" int 0 int 2 ">>),
...
test(14) -> 
    ...
    Stx5 = keys:sign(Ctx5, Accounts5),
    SStx5 = testnet_sign:sign_tx(Stx5, NewPub, NewPriv, Accounts5),
    absorb(SStx5),
...
test(15) ->
    ...
    TheySPK = testnet_sign:sign_tx(SPK, NewPub, NewPriv, Accounts3),
