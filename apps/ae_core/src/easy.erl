...
create_account(NewPub, Amount) ->
    io:fwrite("easy create account \n"),
    {Trees, _, _} = tx_pool:data(),
    Accounts = trees:accounts(Trees),
    Governance = trees:governance(Trees),
    Cost = governance:get_value(ca, Governance),
    create_account(pub2bin(NewPub), Amount, ?Fee + Cost).
create_account(NewPub, Amount, Fee) ->
    F = fun(Trees) ->
		create_account_tx:make(pub2bin(NewPub), to_int(Amount), Fee, pub2bin(keys:pubkey()), Trees) end,
    tx_maker(F).
spend(Pub0, Amount) ->
    Pub = pub2bin(Pub0),
    K = pub2bin(keys:pubkey()),
    if 
	Pub == K -> io:fwrite("you can't spend money to yourself\n");
	true -> 
	    A = to_int(Amount),
	    {Trees, _, _} = tx_pool:data(),
	    Governance = trees:governance(Trees),
	    Cost = governance:get_value(spend, Governance),
	    spend(Pub, A, ?Fee+Cost)
    end.
spend(ID, Amount, Fee) ->
    F = fun(Trees) ->
		spend_tx:make(pub2bin(ID), Amount, Fee, pub2bin(keys:pubkey()), Trees, []) end,
    tx_maker(F).
...
delete_account(ID) ->
    ...
    delete_account(pub2bin(ID), ?Fee+Cost).
delete_account(ID, Fee) ->
    F = fun(Trees) ->
		delete_account_tx:make(pub2bin(keys:pubkey()), pub2bin(ID), Fee, Trees) end,
    tx_maker(F).
...
repo_account(ID) ->   
    ...
    repo_account(pub2bin(ID), ?Fee+Cost).
repo_account(ID, Fee) ->   
    F = fun(Trees) ->
		repo_tx:make(pub2bin(ID), Fee, keys:pubkey(), Trees) end,
    tx_maker(F).
...
new_channel_tx(CID, Acc2, Bal1, Bal2, Entropy, Delay) ->
    ...
    new_channel_tx(CID, pub2bin(Acc2), Bal1, Bal2, Entropy, ?Fee+Cost, Delay).
new_channel_tx(CID, Acc2, Bal1, Bal2, Entropy, Fee, Delay) ->
    ...
    {Trees, _, _} = tx_pool:data(),
    Accounts = trees:accounts(Trees),
    {Tx, _} = new_channel_tx:make(CID, Trees, pub2bin(keys:pubkey()), pub2bin(Acc2), Bal1, Bal2, Entropy, Delay, Fee),
    keys:sign(Tx, Accounts).
...
new_channel_with_server(IP, Port, CID, Bal1, Bal2, Fee, Delay) ->
    ...
    io:fwrite("about to call new_channel\n"),
...
pull_channel_state(IP, Port) ->
    ...
    talker:talk({channel_sync, pub2bin(keys:pubkey()), Return}, IP, Port),
...
bet_unlock(IP, Port) ->
    ...
    teach_secrets(pub2bin(keys:pubkey()), Secrets, IP, Port),
    {Trees, _, _} = tx_pool:data(),
    Accounts = trees:accounts(Trees),
    talker:talk({channel_sync, pub2bin(keys:pubkey()), keys:sign(SPK, Accounts)}, IP, Port),
    {ok, _CD, ThemSPK} = talker:talk({spk, keys:pubkey()}, IP, Port),
    channel_feeder:update_to_me(ThemSPK, ServerID),
    ok.
teach_secrets(_, [], _, _) -> ok;
teach_secrets(ID0, [{secret, Secret, Code}|Secrets], IP, Port) ->
    ID = pub2bin(ID0),
...
lightning_spend(Recipient, Amount) ->
    ...
    lightning_spend(?IP, ?Port, pub2bin(Recipient), Amount, LFee).
lightning_spend(IP, Port, Recipient, Amount, Fee) ->
    {Code, SS} = secrets:new_lightning(),
    lightning_spend(IP, Port, pub2bin(Recipient), Amount, Fee, Code, SS).
lightning_spend(IP, Port, Recipient, Amount, Fee, Code, SS) ->
    {ok, ServerID} = talker:talk({pubkey}, IP, Port),
    ESS = keys:encrypt([SS, Code], pub2bin(Recipient)),
    SSPK = channel_feeder:make_locked_payment(ServerID, Amount+Fee, Code, []),
    {ok, SSPK2} = talker:talk({locked_payment, SSPK, Amount, Fee, Code, keys:pubkey(), Recipient, ESS}, IP, Port),
    {Trees, _, _} = tx_pool:data(),
    Accounts = trees:accounts(Trees),
    ...
    ok.
channel_manager_update(ServerID0, SSPK2) ->
    %store SSPK2 in channel manager, it is their most recent signature.
    ServerID = pub2bin(ServerID0),
...
channel_timeout() ->
    ...
    {ok, CD} = channel_manager:read(Other),
    CID = channel_feeder:cid(CD),
    {Tx, _} = channel_timeout:make(pub2bin(keys:pubkey()), Trees, CID, Fee),
    Accounts = trees:accounts(Trees),
    Stx = keys:sign(Tx, Accounts),
...
channel_repo(CID, Fee) ->
    F = fun(Trees) ->
		channel_repo_tx:make(pub2bin(keys:pubkey()), CID, Fee, Trees) end,
    tx_maker(F).

channel_solo_close(CID, Fee, SPK, ScriptSig) ->
    F = fun(Trees) ->
		channel_solo_close:make(pub2bin(keys:pubkey()), CID, Fee, SPK, ScriptSig, Trees) end,
    tx_maker(F).

channel_timeout(CID, Fee) ->
    F = fun(Trees) ->
		channel_timeout_tx:make(pub2bin(keys:pubkey()), Trees, CID, Fee) end,
    tx_maker(F).

channel_slash(CID, Fee, SPK, SS) ->
    F = fun(Trees) ->
		channel_slash_tx:make(pub2bin(keys:pubkey()), CID, Fee, SPK, SS, Trees) end,
    tx_maker(F).
...
new_question_oracle(Start, Question, DiffOracleID, ID)->
    ...
    F = fun(Trs) ->
		oracle_new_tx:make(pub2bin(keys:pubkey()), ?Fee+Cost, Question, Start, ID, Difficulty, DiffOracleID, 0, 0, Trs) end,
    tx_maker(F).
...
new_difficulty_oracle(Fee, Start, ID, Difficulty) ->
    ...
    F = fun(Trees) ->
		oracle_new_tx:make(pub2bin(keys:pubkey()), Fee, <<"">>, Start, ID, Difficulty, 0, 0, 0, Trees) end,
    tx_maker(F).
new_governance_oracle(Start, GovName, GovAmount, DiffOracleID) ->
    ...
    F = fun(Trs) ->
		...
		Cost = governance:get_value(oracle_new, Governance),
		oracle_new_tx:make(pub2bin(keys:pubkey()), ?Fee + Cost, <<>>, Start, ID, Difficulty, DiffOracleID, GovNumber, GovAmount, Trs) end,
    tx_maker(F).
...
oracle_bet(Fee, OID, Type, Amount) ->
    F = fun(Trees) ->
		oracle_bet_tx:make(pub2bin(keys:pubkey()), Fee, OID, Type, to_int(Amount), Trees)
	end,
    tx_maker(F).
oracle_close(OID) ->
    {Trees, _, _} = tx_pool:data(),
...
oracle_close(Fee, OID) ->
    F = fun(Trees) ->
		oracle_close_tx:make(pub2bin(keys:pubkey()), Fee, OID, Trees)
	end,
    tx_maker(F).
oracle_shares(OID) ->
    {Trees, _, _} = tx_pool:data(),
...
oracle_shares(Fee, OID) ->
    F = fun(Trees) ->
		oracle_shares_tx:make(pub2bin(keys:pubkey()), Fee, OID, Trees)
	end,
    tx_maker(F).
...
oracle_unmatched(Fee, OracleID, OrderID) ->
    F = fun(Trees) ->
		oracle_unmatched_tx:make(pub2bin(keys:pubkey()), Fee, OracleID, OrderID, Trees)
	end,
    tx_maker(F).
...
account(ID) ->
    {Trees,_,_} = tx_pool:data(),
    Accounts = trees:accounts(Trees),
    {_, A, _} = accounts:get(ID, Accounts),
    A.
account() -> account(pub2bin(keys:pubkey())).
integer_balance() -> 
    case account() of
	empty -> 0;
	X -> accounts:balance(X)
    end.
...
channel_close(IP, Port, Fee) ->
    ...
    Accounts = trees:accounts(Trees),
    STx = keys:sign(Tx, Accounts),
    {ok, SSTx} = talker:talk({close_channel, CID, pub2bin(keys:pubkey()), SS, STx}, IP, Port),
    tx_pool_feeder:absorb(SSTx),
    0.
...
channel_solo_close(Other) ->
    ...
    SSPK = channel_feeder:them(CD),
    SS = channel_feeder:script_sig_them(CD),
    {Tx, _} = channel_solo_close:make(pub2bin(keys:pubkey()), Fee, keys:sign(SSPK, Accounts), SS, Trees),
    STx = keys:sign(Tx, Accounts),
    tx_pool_feeder:absorb(STx),
    0.
add_peer(IP, Port) ->
...
%address() ->
%    accounts:pub_decode(pubkey()).
%address(Pub) ->
%    accounts:pub_decode(Pub).
new_pubkey(Password) ->    
    keys:new(Password).
test() ->
    {test_response}.
...
keys_new(Password) ->
    ...
    0.
market_match(OID) ->
    io:fwrite("market_match oid is "),
    io:fwrite(integer_to_list(OID)),
    io:fwrite("\n"),
    ...
    io:fwrite("market match price declaration is "),
    io:fwrite(packer:pack(PriceDeclaration)),
    io:fwrite("\n"),
    ...
    CodeKey = market:market_smart_contract_key(OID, Expires, pub2bin(pubkey()), Period, OID),
