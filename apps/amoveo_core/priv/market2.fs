( direction height expires maxprice marketID Period Pubkey -- )
Pubkey ! Period ! MarketID ! MaxPrice ! Expires ! Height ! direction !

macro check_size direction @ int 2 == tuck drop drop if int 10000 swap - < else > then not ;

direction @ int 2 == tuck drop drop
  if int 0
  else int 10000
  then
bet_amount !
...
macro max ( A B -- M )
      2dup > if else swap then drop
;
macro min ( A B -- M )
      2dup > if swap else then drop
;
...
extract call PM1 ! >r >r extract call PM2 !
...
macro evidence ( signed_price_declaration -- delay nonce amount )
      extract call height Period @ - > or_die %require that the SPD was made in the most recent Period
      % drop
...
macro match_order ( signed_price_declaration -- delay nonce amount )
        extract call ( SPD height price portion_matched )
	...
	% PRICE @ flip MaxPrice @ ==
	PRICE @
        direction @ int 2 == if drop drop int 10000 swap - else drop drop then
        MaxPrice @ ==
	if
