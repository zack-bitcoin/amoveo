...
go(Tx, Dict, NewHeight, _) ->
    ...
    case Sink2 of
        <<0:256>> ->
            ...
            MerkleHash2 = hash:doit(contract_evidence_tx:serialize_row(PayoutVector, <<>>)),

            NewPayoutVector = apply_matrix2vector(Matrix, PayoutVector),
            NewMerkleHash = hash:doit(contract_evidence_tx:serialize_row(NewPayoutVector, <<>>)),
            Contract3 = Contract1#contract{
                          result = NewMerkleHash
                         },
            contracts:dict_write(Contract3, Dict2);
        CID3 ->
            {MRoot2, M3} = contract_evidence_tx:make_tree(Matrix2),
            ...
            {MRoot3, M4} = contract_evidence_tx:make_tree(Matrix3),
