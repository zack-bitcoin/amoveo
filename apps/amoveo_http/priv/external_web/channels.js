function channels_main() {
	...
	var trade_amount_info = document.createElement("label");
	...
	var close_channel_button = button_maker2("close channel", function() {
		return;
	});
	var lightning_button = button_maker2("lightning spend", function() {
		return;
	});
	var lightning_amount = document.createElement("INPUT");
	lightning_amount.setAttribute("type", "text");
	var lightning_amount_info = document.createElement("label");
	lightning_amount_info.innerHTML = "amount: ";
	var lightning_to = document.createElement("INPUT");
	lightning_to.setAttribute("type", "text");
	var lightning_to_info = document.createElement("label");
	...
	function save_channel_data() {
		...
		//channel_warning_div.innerHTML = msg("channel state is saved");
		channel_warning_div.appendChild(msg("channel state is saved"));
	}
	...
	function refresh_channels_interfaces2(pubkey, callback) {
		...
		if (read(pubkey) == undefined) {
			...
			height_button.onclick = function() {
				return make_channel_func(pubkey)
			};
			...
			lightning_button.onclick = function() {
				lightning_spend(pubkey);
			};
			channel_balance_button.onclick = function() {
				refresh_balance(pubkey);
			};
	...
	function close_channel_func(server_pubkey) {
		...
		var fun_limit = 400;
		var var_limit = 10000;
		spk_object.spk_run(0, ss, spk, height, 0, fun_limit, var_limit, function(x) {
			close_channel_func2(x, spk, cd, height, ss, server_pubkey);
		});
	}
	...
	function close_channel_func3(channel, height, cd, amount, ss, cid, server_pubkey) {
		var mypub = keys.pub();
		merkle.request_proof("accounts", mypub, function(acc) {
			close_channel_func4(acc, channel, height, cd, amount, mypub, ss, cid, server_pubkey);
		});
	}
	...
	function make_channel_func(pubkey) {
		...
		variable_public_get(["new_channel_tx", acc1, pubkey, amount, bal2, delay, fee], function(x) {
			make_channel_func2(x, amount, bal2, acc1, acc2, delay, expiration, pubkey);
		});
	}
	...
	function make_channel_func2(tx, amount, bal2, acc1, acc2, delay, expiration, pubkey) {
		...
		if ((!(delay == delay0)) || (!(amount == amount0)) ||
			...
			var stx = keys.sign(tx);
			var sspk = keys.sign(spk);
			variable_public_get(["new_channel", stx, sspk, expiration], function(x) {
	...
	function ss_to_external(ss) {
		return ["ss",
			...
			([-6]).concat(ss.prove),
			([-6]).concat(ss.meta)
		]; //prove and meta should start with -6.
	}
