...
-export([start_link/0,code_change/3,handle_call/3,handle_cast/2,handle_info/2,init/1,terminate/2, absorb/5,dump/0,secrets/0,accounts/0,channels/0,txs/0,test/0]).
-record(f, {txs = [], accounts = dict:new(), channels = dict:new(), secrets = dict:new()}).
init(ok) -> 
    block_tree:save(block:genesis()),%save is like absorb, but we don't check to see if the block is valid.
...
handle_cast({absorb, NewChannels, NewAccounts, NewSecrets, Txs}, _) ->
    {noreply, #f{txs = Txs, accounts = NewAccounts, channels = NewChannels, secrets = NewSecrets}}.
